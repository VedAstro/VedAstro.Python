...
def BadLunarMonthForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.BadLunarMonthForBuilding(time, person)


def GoodSunSignForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.GoodSunSignForBuilding(time, person)


def BadSunSignForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.BadSunSignForBuilding(time, person)


def GoodLunarDayForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.GoodLunarDayForBuilding(time, person)


def GoodWeekDayForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.GoodWeekDayForBuilding(time, person)


def BadLunarPhaseForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.BadLunarPhaseForBuilding(time, person)


def BadWeekDayForBuilding(time: Time, person: Person):
    return libray.EventCalculatorMethods.BadWeekDayForBuilding(time, person)


def BadWeekDayForRepairs(time: Time, person: Person):
    return libray.EventCalculatorMethods.BadWeekDayForRepairs(time, person)


def GoodYogaForRepairs(time: Time, person: Person):
    return libray.EventCalculatorMethods.GoodYogaForRepairs(time, person)


def GoodYogaForRepairs2(time: Time, person: Person):
    return libray.EventCalculatorMethods.GoodYogaForRepairs2(time, person)


def GocharaSummary(time: Time, person: Person):
    return libray.EventCalculatorMethods.GocharaSummary(time, person)


def SunGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse1(time, person)


def SunGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse2(time, person)


def SunGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse3(time, person)


def SunGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse4(time, person)


def SunGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse5(time, person)


def SunGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse6(time, person)


def SunGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse7(time, person)


def SunGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse8(time, person)


def SunGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse9(time, person)


def SunGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse10(time, person)


def SunGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse11(time, person)


def SunGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunGocharaInHouse12(time, person)


def MoonGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse1(time, person)


def MoonGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse2(time, person)


def MoonGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse3(time, person)


def MoonGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse4(time, person)


def MoonGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse5(time, person)


def MoonGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse6(time, person)


def MoonGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse7(time, person)


def MoonGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse8(time, person)


def MoonGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse9(time, person)


def MoonGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse10(time, person)


def MoonGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse11(time, person)


def MoonGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonGocharaInHouse12(time, person)


def MarsGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse1(time, person)


def MarsGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse2(time, person)


def MarsGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse3(time, person)


def MarsGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse4(time, person)


def MarsGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse5(time, person)


def MarsGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse6(time, person)


def MarsGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse7(time, person)


def MarsGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse8(time, person)


def MarsGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse9(time, person)


def MarsGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse10(time, person)


def MarsGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse11(time, person)


def MarsGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsGocharaInHouse12(time, person)


def MercuryGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse1(time, person)


def MercuryGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse2(time, person)


def MercuryGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse3(time, person)


def MercuryGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse4(time, person)


def MercuryGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse5(time, person)


def MercuryGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse6(time, person)


def MercuryGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse7(time, person)


def MercuryGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse8(time, person)


def MercuryGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse9(time, person)


def MercuryGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse10(time, person)


def MercuryGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse11(time, person)


def MercuryGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryGocharaInHouse12(time, person)


def JupiterGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse1(time, person)


def JupiterGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse2(time, person)


def JupiterGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse3(time, person)


def JupiterGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse4(time, person)


def JupiterGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse5(time, person)


def JupiterGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse6(time, person)


def JupiterGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse7(time, person)


def JupiterGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse8(time, person)


def JupiterGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse9(time, person)


def JupiterGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse10(time, person)


def JupiterGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse11(time, person)


def JupiterGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterGocharaInHouse12(time, person)


def VenusGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse1(time, person)


def VenusGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse2(time, person)


def VenusGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse3(time, person)


def VenusGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse4(time, person)


def VenusGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse5(time, person)


def VenusGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse6(time, person)


def VenusGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse7(time, person)


def VenusGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse8(time, person)


def VenusGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse9(time, person)


def VenusGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse10(time, person)


def VenusGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse11(time, person)


def VenusGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusGocharaInHouse12(time, person)


def SaturnGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse1(time, person)


def SaturnGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse2(time, person)


def SaturnGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse3(time, person)


def SaturnGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse4(time, person)


def SaturnGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse5(time, person)


def SaturnGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse6(time, person)


def SaturnGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse7(time, person)


def SaturnGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse8(time, person)


def SaturnGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse9(time, person)


def SaturnGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse10(time, person)


def SaturnGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse11(time, person)


def SaturnGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnGocharaInHouse12(time, person)


def RahuGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse1(time, person)


def RahuGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse2(time, person)


def RahuGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse3(time, person)


def RahuGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse4(time, person)


def RahuGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse5(time, person)


def RahuGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse6(time, person)


def RahuGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse7(time, person)


def RahuGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse8(time, person)


def RahuGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse9(time, person)


def RahuGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse10(time, person)


def RahuGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse11(time, person)


def RahuGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuGocharaInHouse12(time, person)


def KetuGocharaInHouse1(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse1(time, person)


def KetuGocharaInHouse2(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse2(time, person)


def KetuGocharaInHouse3(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse3(time, person)


def KetuGocharaInHouse4(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse4(time, person)


def KetuGocharaInHouse5(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse5(time, person)


def KetuGocharaInHouse6(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse6(time, person)


def KetuGocharaInHouse7(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse7(time, person)


def KetuGocharaInHouse8(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse8(time, person)


def KetuGocharaInHouse9(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse9(time, person)


def KetuGocharaInHouse10(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse10(time, person)


def KetuGocharaInHouse11(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse11(time, person)


def KetuGocharaInHouse12(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuGocharaInHouse12(time, person)


def AriesSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesSunDasa(time, person)


def TaurusSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusSunDasa(time, person)


def GeminiSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiSunDasa(time, person)


def CancerSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerSunDasa(time, person)


def LeoSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoSunDasa(time, person)


def VirgoSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoSunDasa(time, person)


def LibraSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraSunDasa(time, person)


def ScorpioSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioSunDasa(time, person)


def SagittariusSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusSunDasa(time, person)


def CapricornusSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusSunDasa(time, person)


def AquariusSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusSunDasa(time, person)


def PiscesSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesSunDasa(time, person)


def AriesMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesMoonDasa(time, person)


def TaurusMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusMoonDasa(time, person)


def GeminiMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiMoonDasa(time, person)


def CancerMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerMoonDasa(time, person)


def LeoMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoMoonDasa(time, person)


def VirgoMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoMoonDasa(time, person)


def LibraMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraMoonDasa(time, person)


def ScorpioMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioMoonDasa(time, person)


def SagittariusMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusMoonDasa(time, person)


def CapricornusMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusMoonDasa(time, person)


def AquariusMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusMoonDasa(time, person)


def PiscesMoonDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesMoonDasa(time, person)


def AriesMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesMarsDasa(time, person)


def TaurusMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusMarsDasa(time, person)


def GeminiMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiMarsDasa(time, person)


def CancerMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerMarsDasa(time, person)


def LeoMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoMarsDasa(time, person)


def VirgoMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoMarsDasa(time, person)


def LibraMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraMarsDasa(time, person)


def ScorpioMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioMarsDasa(time, person)


def SagittariusMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusMarsDasa(time, person)


def CapricornusMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusMarsDasa(time, person)


def AquariusMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusMarsDasa(time, person)


def PiscesMarsDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesMarsDasa(time, person)


def AriesRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesRahuDasa(time, person)


def TaurusRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusRahuDasa(time, person)


def GeminiRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiRahuDasa(time, person)


def CancerRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerRahuDasa(time, person)


def LeoRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoRahuDasa(time, person)


def VirgoRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoRahuDasa(time, person)


def LibraRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraRahuDasa(time, person)


def ScorpioRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioRahuDasa(time, person)


def SagittariusRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusRahuDasa(time, person)


def CapricornusRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusRahuDasa(time, person)


def AquariusRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusRahuDasa(time, person)


def PiscesRahuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesRahuDasa(time, person)


def AriesJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesJupiterDasa(time, person)


def TaurusJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusJupiterDasa(time, person)


def GeminiJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiJupiterDasa(time, person)


def CancerJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerJupiterDasa(time, person)


def LeoJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoJupiterDasa(time, person)


def VirgoJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoJupiterDasa(time, person)


def LibraJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraJupiterDasa(time, person)


def ScorpioJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioJupiterDasa(time, person)


def SagittariusJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusJupiterDasa(time, person)


def CapricornusJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusJupiterDasa(time, person)


def AquariusJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusJupiterDasa(time, person)


def PiscesJupiterDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesJupiterDasa(time, person)


def AriesSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesSaturnDasa(time, person)


def TaurusSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusSaturnDasa(time, person)


def GeminiSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiSaturnDasa(time, person)


def CancerSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerSaturnDasa(time, person)


def LeoSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoSaturnDasa(time, person)


def VirgoSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoSaturnDasa(time, person)


def LibraSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraSaturnDasa(time, person)


def ScorpioSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioSaturnDasa(time, person)


def SagittariusSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusSaturnDasa(time, person)


def CapricornusSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusSaturnDasa(time, person)


def AquariusSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusSaturnDasa(time, person)


def PiscesSaturnDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesSaturnDasa(time, person)


def AriesMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesMercuryDasa(time, person)


def TaurusMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusMercuryDasa(time, person)


def GeminiMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiMercuryDasa(time, person)


def CancerMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerMercuryDasa(time, person)


def LeoMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoMercuryDasa(time, person)


def VirgoMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoMercuryDasa(time, person)


def LibraMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraMercuryDasa(time, person)


def ScorpioMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioMercuryDasa(time, person)


def SagittariusMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusMercuryDasa(time, person)


def CapricornusMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusMercuryDasa(time, person)


def AquariusMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusMercuryDasa(time, person)


def PiscesMercuryDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesMercuryDasa(time, person)


def AriesKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesKetuDasa(time, person)


def TaurusKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusKetuDasa(time, person)


def GeminiKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiKetuDasa(time, person)


def CancerKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerKetuDasa(time, person)


def LeoKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoKetuDasa(time, person)


def VirgoKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoKetuDasa(time, person)


def LibraKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraKetuDasa(time, person)


def ScorpioKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioKetuDasa(time, person)


def SagittariusKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusKetuDasa(time, person)


def CapricornusKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusKetuDasa(time, person)


def AquariusKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusKetuDasa(time, person)


def PiscesKetuDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesKetuDasa(time, person)


def AriesVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AriesVenusDasa(time, person)


def TaurusVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.TaurusVenusDasa(time, person)


def GeminiVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.GeminiVenusDasa(time, person)


def CancerVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CancerVenusDasa(time, person)


def LeoVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LeoVenusDasa(time, person)


def VirgoVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.VirgoVenusDasa(time, person)


def LibraVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LibraVenusDasa(time, person)


def ScorpioVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ScorpioVenusDasa(time, person)


def SagittariusVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SagittariusVenusDasa(time, person)


def CapricornusVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.CapricornusVenusDasa(time, person)


def AquariusVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.AquariusVenusDasa(time, person)


def PiscesVenusDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.PiscesVenusDasa(time, person)


def SunDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaSunBhukti(time, person)


def SunDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaMoonBhukti(time, person)


def SunDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaMarsBhukti(time, person)


def SunDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaRahuBhukti(time, person)


def SunDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaJupiterBhukti(time, person)


def SunDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaSaturnBhukti(time, person)


def SunDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaMercuryBhukti(time, person)


def SunDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaKetuBhukti(time, person)


def SunDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunDasaVenusBhukti(time, person)


def MoonDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaSunBhukti(time, person)


def MoonDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaMoonBhukti(time, person)


def MoonDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaMarsBhukti(time, person)


def MoonDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaRahuBhukti(time, person)


def MoonDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaJupiterBhukti(time, person)


def MoonDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaSaturnBhukti(time, person)


def MoonDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaMercuryBhukti(time, person)


def MoonDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaKetuBhukti(time, person)


def MoonDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonDasaVenusBhukti(time, person)


def MarsDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaSunBhukti(time, person)


def MarsDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaMoonBhukti(time, person)


def MarsDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaMarsBhukti(time, person)


def MarsDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaRahuBhukti(time, person)


def MarsDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaJupiterBhukti(time, person)


def MarsDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaSaturnBhukti(time, person)


def MarsDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaMercuryBhukti(time, person)


def MarsDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaKetuBhukti(time, person)


def MarsDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsDasaVenusBhukti(time, person)


def RahuDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaSunBhukti(time, person)


def RahuDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaMoonBhukti(time, person)


def RahuDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaMarsBhukti(time, person)


def RahuDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaRahuBhukti(time, person)


def RahuDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaJupiterBhukti(time, person)


def RahuDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaSaturnBhukti(time, person)


def RahuDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaMercuryBhukti(time, person)


def RahuDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaKetuBhukti(time, person)


def RahuDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuDasaVenusBhukti(time, person)


def JupiterDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaSunBhukti(time, person)


def JupiterDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaMoonBhukti(time, person)


def JupiterDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaMarsBhukti(time, person)


def JupiterDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaRahuBhukti(time, person)


def JupiterDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaJupiterBhukti(time, person)


def JupiterDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaSaturnBhukti(time, person)


def JupiterDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaMercuryBhukti(time, person)


def JupiterDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaKetuBhukti(time, person)


def JupiterDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterDasaVenusBhukti(time, person)


def SaturnDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaSunBhukti(time, person)


def SaturnDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaMoonBhukti(time, person)


def SaturnDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaMarsBhukti(time, person)


def SaturnDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaRahuBhukti(time, person)


def SaturnDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaJupiterBhukti(time, person)


def SaturnDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaSaturnBhukti(time, person)


def SaturnDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaMercuryBhukti(time, person)


def SaturnDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaKetuBhukti(time, person)


def SaturnDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnDasaVenusBhukti(time, person)


def MercuryDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaSunBhukti(time, person)


def MercuryDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaMoonBhukti(time, person)


def MercuryDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaMarsBhukti(time, person)


def MercuryDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaRahuBhukti(time, person)


def MercuryDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaJupiterBhukti(time, person)


def MercuryDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaSaturnBhukti(time, person)


def MercuryDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaMercuryBhukti(time, person)


def MercuryDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaKetuBhukti(time, person)


def MercuryDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryDasaVenusBhukti(time, person)


def KetuDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaSunBhukti(time, person)


def KetuDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaMoonBhukti(time, person)


def KetuDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaMarsBhukti(time, person)


def KetuDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaRahuBhukti(time, person)


def KetuDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaJupiterBhukti(time, person)


def KetuDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaSaturnBhukti(time, person)


def KetuDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaMercuryBhukti(time, person)


def KetuDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaKetuBhukti(time, person)


def KetuDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuDasaVenusBhukti(time, person)


def VenusDasaSunBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaSunBhukti(time, person)


def VenusDasaMoonBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaMoonBhukti(time, person)


def VenusDasaMarsBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaMarsBhukti(time, person)


def VenusDasaRahuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaRahuBhukti(time, person)


def VenusDasaJupiterBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaJupiterBhukti(time, person)


def VenusDasaSaturnBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaSaturnBhukti(time, person)


def VenusDasaMercuryBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaMercuryBhukti(time, person)


def VenusDasaKetuBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaKetuBhukti(time, person)


def VenusDasaVenusBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusDasaVenusBhukti(time, person)


def SunBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiSunAntaram(time, person)


def MoonBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiSunAntaram(time, person)


def MarsBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiSunAntaram(time, person)


def RahuBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiSunAntaram(time, person)


def JupiterBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiSunAntaram(time, person)


def SaturnBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiSunAntaram(time, person)


def MercuryBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiSunAntaram(time, person)


def KetuBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiSunAntaram(time, person)


def VenusBhuktiSunAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiSunAntaram(time, person)


def SunBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiMoonAntaram(time, person)


def MoonBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiMoonAntaram(time, person)


def MarsBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiMoonAntaram(time, person)


def RahuBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiMoonAntaram(time, person)


def JupiterBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiMoonAntaram(time, person)


def SaturnBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiMoonAntaram(time, person)


def MercuryBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiMoonAntaram(time, person)


def KetuBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiMoonAntaram(time, person)


def VenusBhuktiMoonAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiMoonAntaram(time, person)


def SunBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiMarsAntaram(time, person)


def MoonBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiMarsAntaram(time, person)


def MarsBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiMarsAntaram(time, person)


def RahuBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiMarsAntaram(time, person)


def JupiterBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiMarsAntaram(time, person)


def SaturnBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiMarsAntaram(time, person)


def MercuryBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiMarsAntaram(time, person)


def KetuBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiMarsAntaram(time, person)


def VenusBhuktiMarsAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiMarsAntaram(time, person)


def SunBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiRahuAntaram(time, person)


def MoonBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiRahuAntaram(time, person)


def MarsBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiRahuAntaram(time, person)


def RahuBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiRahuAntaram(time, person)


def JupiterBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiRahuAntaram(time, person)


def SaturnBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiRahuAntaram(time, person)


def MercuryBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiRahuAntaram(time, person)


def KetuBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiRahuAntaram(time, person)


def VenusBhuktiRahuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiRahuAntaram(time, person)


def SunBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiJupiterAntaram(time, person)


def MoonBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiJupiterAntaram(time, person)


def MarsBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiJupiterAntaram(time, person)


def RahuBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiJupiterAntaram(time, person)


def JupiterBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiJupiterAntaram(time, person)


def SaturnBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiJupiterAntaram(time, person)


def MercuryBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiJupiterAntaram(time, person)


def KetuBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiJupiterAntaram(time, person)


def VenusBhuktiJupiterAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiJupiterAntaram(time, person)


def SunBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiSaturnAntaram(time, person)


def MoonBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiSaturnAntaram(time, person)


def MarsBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiSaturnAntaram(time, person)


def RahuBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiSaturnAntaram(time, person)


def JupiterBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiSaturnAntaram(time, person)


def SaturnBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiSaturnAntaram(time, person)


def MercuryBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiSaturnAntaram(time, person)


def KetuBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiSaturnAntaram(time, person)


def VenusBhuktiSaturnAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiSaturnAntaram(time, person)


def SunBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiMercuryAntaram(time, person)


def MoonBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiMercuryAntaram(time, person)


def MarsBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiMercuryAntaram(time, person)


def RahuBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiMercuryAntaram(time, person)


def JupiterBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiMercuryAntaram(time, person)


def SaturnBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiMercuryAntaram(time, person)


def MercuryBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiMercuryAntaram(time, person)


def KetuBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiMercuryAntaram(time, person)


def VenusBhuktiMercuryAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiMercuryAntaram(time, person)


def SunBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiKetuAntaram(time, person)


def MoonBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiKetuAntaram(time, person)


def MarsBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiKetuAntaram(time, person)


def RahuBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiKetuAntaram(time, person)


def JupiterBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiKetuAntaram(time, person)


def SaturnBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiKetuAntaram(time, person)


def MercuryBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiKetuAntaram(time, person)


def KetuBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiKetuAntaram(time, person)


def VenusBhuktiKetuAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiKetuAntaram(time, person)


def SunBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBhuktiVenusAntaram(time, person)


def MoonBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MoonBhuktiVenusAntaram(time, person)


def MarsBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MarsBhuktiVenusAntaram(time, person)


def RahuBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.RahuBhuktiVenusAntaram(time, person)


def JupiterBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.JupiterBhuktiVenusAntaram(time, person)


def SaturnBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.SaturnBhuktiVenusAntaram(time, person)


def MercuryBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.MercuryBhuktiVenusAntaram(time, person)


def KetuBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.KetuBhuktiVenusAntaram(time, person)


def VenusBhuktiVenusAntaram(time: Time, person: Person):
    return libray.EventCalculatorMethods.VenusBhuktiVenusAntaram(time, person)
...
def Lord6And8Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Lord6And8Dasa(time, person)


def Lord5And9Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Lord5And9Dasa(time, person)


def Lord5And9DasaBhukti(time: Time, person: Person):
    return libray.EventCalculatorMethods.Lord5And9DasaBhukti(time, person)


def BhuktiDasaLordInBadHouses(time: Time, person: Person):
    return libray.EventCalculatorMethods.BhuktiDasaLordInBadHouses(time, person)


def Lord2Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Lord2Dasa(time, person)


def Lord3Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Lord3Dasa(time, person)


def LagnaLordDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.LagnaLordDasa(time, person)


def Saturn4thDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Saturn4thDasa(time, person)


def Jupiter6thDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.Jupiter6thDasa(time, person)


def ElevatedSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ElevatedSunDasa(time, person)


def SunWithLord9Or10Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunWithLord9Or10Dasa(time, person)


def SunWithLord5Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunWithLord5Dasa(time, person)


def SunWithLord2Dasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunWithLord2Dasa(time, person)


def SunBadPositionDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.SunBadPositionDasa(time, person)


def ExaltatedSunDasa(time: Time, person: Person):
    return libray.EventCalculatorMethods.ExaltatedSunDasa(time, person)
